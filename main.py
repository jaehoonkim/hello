...
app = FastAPI(title="Time API", description="현재 날짜와 시간을 제공하는 REST API", version="1.0.0")
...
async def get_time_format(format: str):
    ...
    if format == "iso":
        return {"format": "iso", "utc": now.isoformat(), "local": local_now.isoformat()}
    elif format == "unix":
        return {
            "format": "unix",
            "timestamp": int(time.time()),
        ...
        raise HTTPException(
            status_code=400, detail=f"지원하지 않는 형식입니다. 사용 가능한 형식: iso, unix, korean"
        )
